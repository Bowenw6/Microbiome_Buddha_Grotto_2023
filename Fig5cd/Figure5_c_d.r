...
infor <- data.frame(SampleID = colnames(ASV_all_ITS), Group = NA, stringsAsFactors = FALSE)
...
ps_ITS <- phyloseq(
  otu_table(as.matrix(ASV_filtered_ITS), taxa_are_rows = TRUE),
...
saveRDS(tab.r_ITS, file = paste0(folder,"/","ITS_network_all.pip.rds"))

tab.r_ITS<-readRDS(file = paste0(folder,"/","ITS_network_all.pip.rds"))

dat_ITS <- tab.r_ITS[[2]]
cortab_ITS <- dat_ITS$net.cor.matrix$cortab
node_all <- dat_ITS$net.cor.matrix$node 
edge_all <- dat_ITS$net.cor.matrix$edge
...
groups <- sort(unique(as.character(infor$Group)))

present_ASVs <- lapply(groups, function(g){
  samp_ids <- rownames(infor)[infor$Group == g]
  asv_ids <- rownames(ASV_filtered_ITS)[rowSums(ASV_filtered_ITS[, samp_ids, drop = FALSE]) > 0]
...
for (g in groups) {
  ...
  if ("OTU_1" %in% names(edge_all) & "OTU_2" %in% names(edge_all)) {
    ...
    edg <- cortab_ITS %>% filter(OTU_1 %in% present & OTU_2 %in% present)
  }
  ...
  if (all(c("OTU_1", "OTU_2") %in% names(edg))) {
    ...
    stop(paste0("无法识别边表的端点列名（组 ", g, "）。请检查 edge_all/cortab_ITS 的列名。"))
  }
  ...
  nodes_3 <- nodes_2 %>% 
    select(name,Kingdom,Phylum,Class,
...
View(nodes_list$Black %>% select(name,Kingdom,Phylum,Class,
       Order,Family,Genus,Species)
)
...
p_test <- ggraph(g, layout = "fr") + 
  ...
  scale_fill_manual(values = c(
    "Ascomycota" = "#a6cee3",
    "unassigned" = "#fb9a99",
    "Basidiomycota" = "#e31a1c",
    "Fungi_phy_Incertae_sedis" = "#ff7f00",
    "Chytridiomycota" = "#6a3d9a",
    "Mortierellomycota" = "#1f78b4",
    "Aphelidiomycota" = "#33a02c",
    "Others" = "gray90"
...
ggsave(
  filename = paste0(folder, "/ITS_network_combined_unified_legend.pdf"),
...
ggsave(file.path(folder, "ITS_combined_ggpubr.pdf"),
